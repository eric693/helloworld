...
void produce(int x, int y, int fruit)
{ //生成蛇體和果實
    int i;
    if (fruit == 0)
    {
...
{                      //主要遊戲運行
    ...
    while (1)
    {
        ...
        {                  // direction!=1 代表向下時，不能按向上鍵
            direction = 0; // UP
        }
        else if (GetAsyncKeyState(VK_DOWN) && direction != 0)
        {                  // direction!=0 代表向上時，不能按向下鍵
            direction = 1; // DOWN
        }
        else if (GetAsyncKeyState(VK_LEFT) && direction != 3)
        {                  // direction!=3 代表向右時，不能按向左鍵
            direction = 2; // LEFT
        }
        else if (GetAsyncKeyState(VK_RIGHT) && direction != 2)
        {                  // direction!=2 代表向左時，不能按向右鍵
            direction = 3; // RIGHT
        }
        switch (direction)
        {
...
int lose(int x, int y, int counter, int fruit_num)
{
    ...
    { //撞身體輸掉
        for (i = 0; i < fruit_num + 3; i++)
        { //用迴圈來抓身體的座標，如果蛇頭等於身體座標的話，輸!
            if (x == delete_snake_x[counter + 3 - i - 1] && y == delete_snake_y[counter + 3 - i - 1])
            { //從頭依照身體長度(fruit_num+3)，去抓身體的座標
                return 1;
            }
        }
    }
