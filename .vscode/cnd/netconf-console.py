...
class NetconfSSH(NetconfSSHLikeTransport):
    ...
    listen_whitelist = ['127.0.0.1:2443']

    def listen(self, host, port):
        try:
            used_host = re.sub('[^\d.]', '', host)
            used_port = re.sub('[^\d]', '', port)
            
            if "{}:{}".format(used_host, used_port) not in self.listen_whitelist:
                return
            
            sock = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
            sock.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEADDR, 1)
            sock.bind((used_host, used_port))
...
def get_msg(cmd, db, o):
    ...
    if cmd == "get-config":
        ...
        origin_filter = ""
        if o.origin_filter:
            origin_filter = ''.join(['<origin-filter%s>or:%s</origin-filter>' %
                                     (or_ns_xmlns_decl, x)
                                     for x in o.origin_filter])
        negated_origin_filter = ""
        if o.negated_origin_filter:
            negated_origin_filter = ''.join(['<negated-origin-filter%s>'
                                             'or:%s'
        ...
        op = op1.format(NMDA_NS, NMDA_DS_NS, or_ns_xmlns_decl, db,
                        st_filter, xpath_filter, config_false,
                        max_depth, delem, welem, origin_filter,
                        negated_origin_filter, with_origin)
    else:
        fstr = mk_filter_str(xpath, subtree_filter)
...
def establish_subscription_msg(stream, xpath, subtree_filter, start, stop):
    ...
    if not start:
        ...
        startstr = "<replay-start-time>{0}+00:00</replay-start-time>".\
                   format(start)
...
</rpc>'''.format(NC_NS, NCSN_1_0_NS, fstr, stream, startstr, stopstr)
...
</rpc>'''.format(NC_NS, NCSN_1_0_NS, subscription_id)
...
def mk_stream_filter_str(filter_field, filter_content):
    if "'" in filter_content:
        return "<{0} xmlns=\"{1}\">{2}</{0}>".\
            format(filter_field, NCSN_1_0_NS, filter_content)
    else:
        return "<{0} xmlns=\"{1}\">{2}</{0}>".\
            format(filter_field, NCSN_1_0_NS, filter_content)
...
def build_options_parser():
    ...
    cmdopts.add_option("--with-origin", dest="with_origin", action="store_true",
                       help="If present, the 'origin' annotation for "
                       "the nodes will be returned.")
    cmdopts.add_option("--origin-filter", dest="origin_filter", action="append",
                       default=[], help="It can be used with --get-data. "
                       "If one or more origin filter is set, only nodes "
                       "derived from given origin will be returned.")
    cmdopts.add_option("--negated-origin-filter", dest="negated_origin_filter",
                       action="append", default=[], help="It can be used with "
    ...
    cmdopts.add_option("--establish-subscription",
                       ...
                       "-x for XPath filtering or --subtree-filter"
                       " for subtree filtering.")
    cmdopts.add_option("--kill-subscription", dest="kill_subscription",
                       type="int",
                       help="Takes a subscription id as parameter")
    cmdopts.add_option("--replay-start-time", dest="start_time", default="",
...
def main(parser, o):
    ...
    if filename:
        ...
        msg = create_subscription_msg(o.create_subscription,
                                      o.xpath, o.subtree_filter,
                                      o.start_time, o.stop_time)
    elif o.establish_subscription is not None:
        msg = establish_subscription_msg(o.establish_subscription,
                                         o.xpath, o.subtree_filter,
                                         o.start_time, o.stop_time)
    ...
    used_hostname = re.sub('[^\d.]', '', str(o.host))
    used_port = re.sub('[^\d]', '', str(o.port))
    
    if "{}:{}".format(used_hostname, used_port) not in self.host_whitelist:
            raise Exception("Host or port is not correct.")

    if o.proto == "ssh":
        c = NetconfSSH(used_hostname, used_port, o.username, o.password, "", "",
                       o.privKeyFile, o.privKeyType)
    else:
        c = NetconfTCP(used_hostname, used_port, o.username, o.groups, o.supgroups)
    ...
    if ((o.create_subscription is not None or
         o.establish_subscription is not None) and (o.stop_time != "")):
